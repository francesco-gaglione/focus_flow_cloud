...
async fn test_refresh_token_flow() {
    ...
    // 3. Refresh the token
    let refresh_dto = RefreshDto {
        refresh_token: login_response.refresh_token,
    };

    let response = client
        .post(format!("{}/api/auth/refresh", ctx.base_url))
        .json(&refresh_dto)
        .send()
        .await
    ...
    let refresh_response: RefreshResponseDto = response
        .json()
        .await
    ...
    headers.insert(
        reqwest::header::AUTHORIZATION,
        reqwest::header::HeaderValue::from_str(&format!("Bearer {}", refresh_response.token))
            .unwrap(),
    );
    let user_client = reqwest::Client::builder()
        .default_headers(headers)
